window.onload = function () {
    ...
    function addnews(key, key_2) {
        ...
        newdiv.addEventListener('click', function(){
            ...
            for (let i=0;i<posts[posts.length-1].coindict[key][key_2].length;i++){
                const dat=posts[posts.length-1].coindict[key][key_2][i];
    ...
    function buildCalendar() {
        ...
        for (var key in posts[posts.length-1].coindict) {
            var cyear=key.split('-')[0];
            var cmonth=key.split('-')[1];
            if (cyear == tyear && cmonth == tmonth) {
                for (var key_2 in posts[posts.length-1].coindict[key]){
                    addnews(key, key_2);
                }
            }
        }
